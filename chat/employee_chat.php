...
$stmt = $conn->prepare("SELECT c.*, cl.name as client_name, p.is_active as product_active
                       FROM complaints c 
                       JOIN assignments a ON c.complaint_id = a.complaint_id 
                       JOIN clients cl ON c.client_id = cl.client_id 
                       LEFT JOIN products p ON c.product_id = p.product_id
                       WHERE c.complaint_id = ? AND a.employee_id = ?");
$stmt->execute([$complaint_id, $employee_id]);
...
<head>
    <meta charset="UTF-8">
    <title>Employee Chat - Complaint #<?php echo htmlspecialchars($complaint_id); ?></title>
...
<div class="chat-container">
    <a href="employee/emp_dash.php" class="back-button">← Back to Dashboard</a>
    
    <div class="chat-header">
        <h2>Chat for Complaint #<?php echo htmlspecialchars($complaint_id); ?></h2>
        <h3>Client: <?php echo htmlspecialchars($complaint['client_name']); ?></h3>
        <p>Description: <?php echo htmlspecialchars($complaint['description']); ?></p>
    </div>

    <div class="chat-messages">
        <?php foreach ($messages as $msg): ?>
            <div class="message <?php echo $msg['sender'] === 'employee' ? 'employee-message' : 'client-message'; ?>">
            ...
            .then(data => {
                if (data.messages) {
                    ...
                    data.messages.forEach(msg => {
                        ...
                        messageDiv.classList.add('message', msg.sender === 'employee' ? 'employee-message' : 'client-message');
                        messageDiv.innerHTML = `<p>${msg.message}</p><span>${msg.sent_at}</span>`;
