...
function updateClientStatusForProduct($conn, $product_id, $is_active) {
    try {
        ...
        // Update client status if the columns exist
        ...
        // Update complaints status when product is deactivated
        if (!$is_active && columnExists($conn, 'complaints', 'is_inactive')) {
            // Mark all complaints for this product as inactive
            $stmt = $conn->prepare("
                UPDATE complaints 
                SET is_inactive = TRUE, 
                    inactive_reason = 'Product deactivated',
                    inactivated_at = NOW()
                WHERE product_id = ? AND is_inactive = FALSE
            ");
        ...
        error_log("Error in updateClientStatusForProduct: " . $e->getMessage());
        return false;
    }
}
